...
* Фильтры: выбор кластеров, диапазонов мощности,
...
# Вспомогательная функция для отображения номера группы в формуле.
def subscript(key: str) -> str:
    mapping = {
        'technical': '₁',
        'economic': '₂',
        'operational': '₃',
...
def calculate_scores(
    ...
    *,
    ...
    # Базовые веса групп (если не переданы явно). Эти значения могут быть
    # переопределены при вызове функции для конкретной категории потребителя.
    ...
    *,
    ...
    if len(top_df) >= 3:
        ...
        radar_fig.update_layout(
            ...
            showlegend=True,
            legend=dict(font=dict(color='#f5b461')),
            plot_bgcolor='#0d1f32',
            paper_bgcolor='#0d1f32',
            font_color='#f5b461',
            height=350,
    ...
    # Формула итоговой оценки с актуальными весами групп.
    colors = {
        'technical': '#64b5f6',
        'economic': '#4db6ac',
        'operational': '#ffb74d',
        'environmental': '#ba68c8',
        'sanction': '#f06292',
    }
    # Если веса групп не переданы, используем базовые значения по умолчанию.
    ...
    # Формируем HTML‑строку с формулой
    formula_parts = []
    for group_key in ['technical', 'economic', 'operational', 'environmental', 'sanction']:
        w = weights.get(group_key, 0)
        color = colors.get(group_key, '#cccccc')
        # заменяем точку на запятую для русской локали
        w_str = f"{w:.2f}".replace('.', ',')
        formula_parts.append(
            f"<span style=\"color:{color}; font-weight:600;\">{w_str}·G{subscript(group_key)}</span>"
        )
    # Соединяем части формулы через плюс
...
def main():
    ...
    st.markdown(
        ...
        html, body, [class*="st-"] {
            background-color: #0d1f32; /* тёмно‑синий фон */
            color: #f5b461; /* бледно‑оранжевый текст */
        ...
        /* Цвет текста в слайдерах */
        div[data-testid="stSlider"] div {
            color: #f5b461;
        }
        /* Цвет трека и ползунка слайдеров */
        [data-baseweb="slider"] .track {
            background-color: #2a4468 !important;
        }
        [data-baseweb="slider"] .thumb {
            background-color: #f5b461 !important;
            border-color: #f5b461 !important;
        }
    ...
    # Диапазон мощности и часы работы в год
    ...
    # Применяем фильтр (по кластеру и мощности). Фильтра по полному ресурсу больше нет.
    ...
    if filtered.empty:
        ...
        return
    # Веса групп критериев в зависимости от выбранной категории потребителя.
    # Каждая запись должна суммироваться к 1.0 и иметь санкционную долю не менее 0.2.
    category_weights = {
        'ЖКХ': {'technical': 0.35, 'economic': 0.25, 'operational': 0.10, 'environmental': 0.10, 'sanction': 0.20},
        'Нефтегаз': {'technical': 0.30, 'economic': 0.30, 'operational': 0.10, 'environmental': 0.10, 'sanction': 0.20},
        'Промышленность': {'technical': 0.32, 'economic': 0.28, 'operational': 0.10, 'environmental': 0.10, 'sanction': 0.20},
        'ГОК': {'technical': 0.30, 'economic': 0.25, 'operational': 0.10, 'environmental': 0.15, 'sanction': 0.20},
        'ЦоД': {'technical': 0.35, 'economic': 0.25, 'operational': 0.10, 'environmental': 0.10, 'sanction': 0.20},
        'Майнинг': {'technical': 0.30, 'economic': 0.40, 'operational': 0.05, 'environmental': 0.05, 'sanction': 0.20},
        'Сельское хозяйство и АПК': {'technical': 0.32, 'economic': 0.28, 'operational': 0.10, 'environmental': 0.10, 'sanction': 0.20},
    }
    selected_weights = category_weights.get(selected_category, None)
    # Отрисовываем дашборд с учётом индивидуальных весов
    render_dashboard(
        filtered,
        hours_per_year=int(hours_per_year),
        gas_price=float(gas_price),
        years=int(years),
        group_weights=selected_weights,
    )
