...
def main():
    ...
    # Загрузить и отобразить декоративное изображение шапки
    try:
        st.image('header_image.png', use_column_width=True)
    except Exception:
        # если изображение отсутствует, выводим только заголовок
        pass
    st.title('ГПУ‑Эксперт')
    st.caption('Суверенный выбор газопоршневых установок')
