...
def main():
    ...
    if df_source is None or df_source.empty:
        ...
        st.markdown('</div>', unsafe_allow_html=True)
        return
    # Предобрабатываем данные сразу для получения нормализованных колонок (имена P_el, R_full и т.д.)
    df_proc_input = preprocess_data(df_source)
    # Категория потребителя
    ...
    clusters = sorted(df_proc_input['Кластер'].dropna().unique().tolist())
    selected_clusters = st.multiselect('Кластеры:', clusters, default=clusters)
    # Слайдеры для диапазонов
    cols = st.columns(3)
    with cols[0]:
        # Используем предобработанные данные для определения диапазона мощности
        if 'P_el' in df_proc_input.columns and not df_proc_input['P_el'].isna().all():
            min_power = float(df_proc_input['P_el'].min())
            max_power = float(df_proc_input['P_el'].max())
        else:
            min_power = 0.5
            max_power = 60.0
    ...
    with cols[1]:
        # Используем предобработанные данные для определения диапазона полного ресурса
        if 'R_full' in df_proc_input.columns and not df_proc_input['R_full'].isna().all():
            min_life = float(df_proc_input['R_full'].min())
            max_life = float(df_proc_input['R_full'].max())
        else:
            min_life = 5.0
            max_life = 35.0
    ...
    # Для фильтрации используем те же предобработанные данные
    df_proc = df_proc_input
    mask = (
